...
It can get more complicated, but for exploratory visualization this often sufficient.
...
Let's plot finsqft vs yearbuilt using a line.
...
Plots created with ggplot are static. However we can make them interactive with the `ggplotly()` function from the plotly package. plotly is an R package that allows you to create interactive visualizations. 

Simply call `ggplotly()` after your plot, or save your plot and call `ggplotly()` on the object name.

For example, recall this plot.
...
Now make the plot interactive with `ggplotly()`. The `plotly::` syntax allows us to use the `ggplotly` function without explicitly loading the plotly package with `library(plotly)`. 
...
We can also save a plot and use `ggplotly()`

```{r}
p <- ggplot(homes_median_sf) +
  aes(x = yearbuilt, y = finsqft) +
  geom_line()
plotly::ggplotly(p)
```
...
If you need to export a plot created by ggplot you can use the `ggsave()` function. For example to save a jpeg version:

```{r}
ggplot(homes_median_sf) +
  aes(x = yearbuilt, y = finsqft) +
  geom_line()
ggsave("median_sf.jpeg")

## set dimensions in cm
ggsave("median_sf.jpeg", width = 20, height = 20, units = "cm")
```

Can also save as png, pdf, tif and others. See `?ggsave`.

You can also use point-and-click in RStudio in the Plots window. Just click the Export button and follow directions in the dialog.

To save an interactive plot created with plotly, use the `saveWidget()` function from the htmlwidgets package. (This package is installed when you install plotly.) The result is a self-contained HTML page that can be shared or hosted. For example, to save the interactive plot we created earlier:

```{r}
w <- plotly::ggplotly(p)
htmlwidgets::saveWidget(w, file = "median_sf.html")
```

This will create an HTML file in your current working directory that you can email to someone, present to an audience, or host on a web site. 
...
* Wickham, H. (2016), _ggplot2: Elegant Graphics for Data Analysis_ (2nd ed), Springer. https://ggplot2-book.org/

* Wickham, H. and Grolemund G. (2017), _R for Data Science_. O'Reilly. http://r4ds.had.co.nz/

* C. Sievert. (2020), _Interactive Web-Based Data Visualization with R, plotly, and shiny_. Chapman and Hall/CRC Florida. https://plotly-r.com
